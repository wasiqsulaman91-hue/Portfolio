...
export function Hero() {
  ...
  useEffect(() => {
    // Safety fallback: ensure wallpaper reveals even if iframe onLoad is delayed
    const timer = setTimeout(() => setSplineLoaded(true), 1200);
    ...
    tl.from(".hero-eyebrow", { opacity: 0, y: 20, duration: 0.7, ease: "power3.out" })
      ...
      .from(".hero-spline", { opacity: 0, x: 80, duration: 1.4, ease: "power2.out" }, 0.2);
  ...
  return (
    <section id="home" className="relative isolate flex min-h-screen items-center overflow-hidden">
      ...
      />
      <div className="grid-lines pointer-events-none absolute inset-0 z-0 opacity-40 [mask-image:radial-gradient(ellipse_at_50%_20%,black,transparent_75%)]" />
      <div className="starfield pointer-events-none absolute inset-0 z-0 opacity-60" />

      <div className="glow-orb glow-orb-float top-[18%] left-[8%] z-0 h-56 w-56 bg-violet/30" />
      <div className="glow-orb glow-orb-float bottom-[12%] left-[38%] z-0 h-72 w-72 bg-glow/20" />
      <div className="glow-orb glow-orb-float top-[8%] right-[12%] z-0 h-64 w-64 bg-ice/15" />

      {/* Spline 3D orb — full-bleed background */}
      <div className="hero-spline pointer-events-none absolute inset-0 z-0">
        <div
          className={`relative h-full w-full transition-opacity duration-1000 ${
            splineLoaded ? "opacity-75 md:opacity-100" : "opacity-0"
          }`}
        >
          <iframe
            ...
            width="100%"
            height="100%"
            allow="fullscreen; autoplay; xr-spatial-tracking; gpu"
