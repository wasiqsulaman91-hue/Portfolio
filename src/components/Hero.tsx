...
export function Hero() {
  ...
  // Heavy WebGL 3D scene is desktop-only: on phones it stalls scrolling and
  // renders with artifacts. Also skipped when the user prefers reduced motion.
  ...
  useEffect(() => {
    ...
    if (light) return;
    // Mount after first paint so the hero text is interactive immediately.
    const id = window.setTimeout(() => setShowSpline(true), 600);
  ...
  useEffect(() => {
    ...
    // Endless orb float is desktop-only — animating three huge blurred layers
    // forever is the main cause of scroll jank on phones.
    let orbs: gsap.core.Tween | null = null;
    if (window.matchMedia("(min-width: 1024px)").matches) {
      orbs = gsap.to(".glow-orb-float", {
        y: -20,
        duration: 3,
        repeat: -1,
        yoyo: true,
  ...
  return (
    <section id="home" className="relative flex min-h-screen items-center overflow-hidden">
      ...
      {/* Spline 3D orb — full-bleed background, desktop only (hidden until loaded) */}
      <div className="hero-spline pointer-events-none absolute inset-0 -z-10 bg-background">
        {showSpline ? (
          ...
          <div
            ...
            style={{
              background:
                "radial-gradient(60% 45% at 72% 42%, color-mix(in oklab, var(--glow) 30%, transparent), transparent 70%), radial-gradient(45% 35% at 30% 70%, color-mix(in oklab, var(--violet) 24%, transparent), transparent 72%)",
            }}
          />
        )}
      </div>
